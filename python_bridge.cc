...
PyObject* LLVMFuzzerMutatePyCallback(PyObject* data, PyObject* args) {
  ...
  /*
  size_t RetLen = LLVMFuzzerMutate((uint8_t *)PyByteArray_AsString(py_value), Size, MaxSize);
  ...
  if (module_name) {
    ...
    if (py_module != NULL) {
      ...
      py_functions[PY_FUNC_INIT] =
        PyObject_GetAttrString(py_module, "init");

      if (!py_functions[PY_FUNC_INIT]
        || !PyCallable_Check(py_functions[PY_FUNC_INIT])) {
        if (PyErr_Occurred())
          PyErr_Print();
        fprintf(stderr, "Error: Cannot find/call init function in"
      ...
      /* Initialise the dharma engine */
      if(!PyObject_CallObject(py_functions[PY_FUNC_INIT], NULL)){
       if (PyErr_Occurred())
          PyErr_Print();
